...
export const requireAnyRole = (roles) => {
  ...
  return (req, res, next) => {
    ...
    if (!allowedRoles.includes(req.user.role)) {
      logSecurityEvent(
        'ROLE_ACCESS_DENIED',
        `User with role ${req.user.role} tried to access resource requiring: ${allowedRoles.join(', ')}`,
        req.user._id,
        req.ip
      );
...
export const requireExpertOrAdmin = requireAnyRole([USER_ROLES.EXPERT, USER_ROLES.ADMIN]);
  ...
  return (req, res, next) => {
    ...
    if (userLevel < requiredLevel) {
      ...
      return res.status(403).json(formatResponse(
        false,
        null,
        `Требуется минимальная роль: ${minRole}`
      ));
    }
  ...
  if (!req.user.canPerformAction()) {
    ...
    if (!req.user.isActive) {
      ...
      const until = req.user.bannedUntil 
        ...
        : ' навсегда';
      message = `Аккаунт заблокирован${until}. Причина: ${req.user.bannedReason || 'не указана'}`;
    }
...
export const attachUserPermissions = (req, res, next) => {
  if (req.user) {
    req.userPermissions = {
      ...
      canPerformAction: req.user.canPerformAction(),
      role: req.user.role
    };
  } else {
    req.userPermissions = {
